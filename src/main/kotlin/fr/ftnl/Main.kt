				...
				val embed = Embed {
					...
					field {
						name = manager.getString(
							LangKey.keyBuilder(this@BotListener, "embed.field1", "title"), "It's a fork :"
						)
					...
					field {
						name = manager.getString(
							LangKey.keyBuilder(this@BotListener, "embed.field3", "title"), "Message content intent :"
						)
					...
					field {
						name = manager.getString(
							LangKey.keyBuilder(this@BotListener, "embed.field4", "title"), "Some statistics :"
						)
				...
				val message = MessageCreateBuilder().setEmbeds(embed).addActionRow(
						...
						Button.link(
								manager.getString(
									...
									LangKey.keyBuilder(this@BotListener, "buttons.site", "label"), "My website"
								)
							).withEmoji(Emoji.fromCustom("invlog_sheep", 763833470232559626L, false))
					).build()
				
				event.reply(message).setEphemeral(event.member?.hasPermission(Permission.MESSAGE_MANAGE) == false)
					.queue()
			}
